...
// Actual ATR from the SIM is not available use this one (Airmont SIM Atr)
UCHAR Scard_Atr[] = {0x3B, 0x9E, 0x96, 0x80, 0x1F, 0xC7, 0x80, 0x31, 0xE0, 0x73, 0xFE, 0x21, 0x1B, 0x66, 0xD0, 0x02, 0x17, 0x8D, 0x11, 0x00, 0x89};
...
RESPONSECODE IFDHPowerICC ( DWORD Lun, DWORD Action, 
  ...
  */
   switch (Action) {
       case IFD_POWER_UP:
       case IFD_RESET:
           memcpy(Atr, &Scard_Atr, sizeof(Scard_Atr));
           *AtrLength = sizeof(Scard_Atr);
           return IFD_SUCCESS;
           break;
       case IFD_POWER_DOWN:
           Atr = 0;
...
/* Print Transmit Error Messaging */
  ...
  for ( i=0; i < lc; i++ ) {
    ...
    if ( CT_data( iTerm, &dad, &sad, lc, TxBuffer, &lr, RxBuffer ) ==
         OK ) {
